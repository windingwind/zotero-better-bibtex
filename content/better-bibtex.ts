...
export class BetterBibTeX {
  ...
  public async startup(reason: Reason): Promise<void> {
    ...
    const ts = (): string => (new Date).toISOString().replace(/Z/, '')
    ...
    orchestrator.add({
      ...
      startup: async () => {
        ...
        log.debug('sqlite: table detect took', -now + (now = Date.now()), 'ms', ts())
        ...
        log.debug('sqlite: citation-key.sql took', -now + (now = Date.now()), 'ms', ts())
        for (const ddl of require('./db/auto-export.sql')) {
          await Zotero.DB.queryAsync(ddl, [], NoParse)
        }
        log.debug('sqlite: auto-export.sql took', -now + (now = Date.now()), 'ms', ts())
        for (const ddl of require('../gen/auto-export-triggers.sql')) {
          await Zotero.DB.queryAsync(ddl, [], NoParse)
        }
        log.debug('sqlite: auto-export-triggers.sql took', -now + (now = Date.now()), 'ms', ts())
        ...
        log.debug('sqlite: table migration took', -now + (now = Date.now()), 'ms', 'now finished', ts())
      },
      shutdown: async () => {
